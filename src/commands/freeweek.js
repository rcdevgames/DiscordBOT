const key = "RGAPI-4363c081-075e-4d9d-bf5b-806d075e603a";
var axios = require('axios');


function championID(id)
{
    switch(id){
...
const request = async ( ) => {
    let stringChamps = "";
    const response = await axios.get("https://br1.api.riotgames.com/lol/platform/v3/champion-rotations?api_key=RGAPI-4363c081-075e-4d9d-bf5b-806d075e603a");
    
    response.data.freeChampionIds.map((x) => {
        stringChamps += "[" + championID(x) + "] ";
    })
    msg.channel.send(stringChamps)
}
request()